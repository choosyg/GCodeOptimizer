...
Part optimizePart(const Part& part) {
	...
	for (size_t i = 1; i < blocks.size(); ++i) {
		...
		* The next z block is colinear as well and at the same positions
		...
		-> der letzte Block muss den R�ckweg machen, sonst bleibt die Rampe �brig
		*/
		...
		Die erste H�lfte der Commands im Block m�ssen nachfolgend nochmal ohne Z anpassung angeh�ngt sein
		...
		*/
		...
		for (size_t i = 0; i < steps; ++i) {
			...
			//Gehe doppelt so tief wie urspr�nglich auf diesem Abschnitt, aber nicht tiefer als wir am Ende ein wollen
